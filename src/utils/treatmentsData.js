// Treatment data for all treatment pages
export const TREATMENTS_DATA = [
  {
    key: "spine",
    routes: ["spine", "spine-treatments"],
    tag: "Spine Treatments",
    title: "Advanced Spine Treatment Solutions",
    image: "/src/assets/treatments/SpineTreatment.jpg",
    description:
      "Expert homeopathic care for spine-related conditions using gentle, effective, and natural treatment methods. Our specialized team is dedicated to helping you achieve a pain-free, active lifestyle.",
    services: {
      title: "Our Spine Treatment Services",
      subtitle:
        "Homeopathic care focused on your spine pain and long-term recovery needs",
      services: [
        "Lumbar spondylosis",
        "Cervical spondylosis",
        "Ankylosing spondylitis",
        "Slip disc",
        "Sciatica",
        "Spondylolisthesis",
      ],
    },
    symptoms: {
      title: "When to Seek Treatment",
      cardTitle: "Common Symptoms:",
      description:
        "If you're experiencing any of these symptoms, it's important to consult with our spine specialists. Early homeopathic treatment can prevent progression and support long-term recovery.",
      symptoms: [
        "Persistent lower back pain",
        "Radiating pain down the legs",
        "Numbness or tingling in extremities",
        "Difficulty walking or standing",
        "Muscle weakness",
        "Limited range of motion",
        "Pain that worsens with activity",
      ],
    },
    treatmentApproach: [
      {
        step: 1,
        title: "Comprehensive Evaluation",
        description:
          "A detailed assessment of symptoms, lifestyle patterns, and medical history to understand the root cause of your spine condition.",
      },
      {
        step: 2,
        title: "Personalized Treatment Plan",
        description:
          "A customized homeopathic care plan based on your specific symptoms, routine, and health goals.",
      },
      {
        step: 3,
        title: "Advanced Homeopathic Care",
        description:
          "Natural, non-invasive remedies designed to reduce pain, ease nerve pressure, and support long-term spine healing.",
      },
      {
        step: 4,
        title: "Recovery & Rehabilitation",
        description:
          "Guidance on posture, daily habits, and simple exercises to ensure sustained recovery and prevent recurrence.",
      },
    ],
    faqs: [
      {
        question: "What causes spine problems?",
        answer:
          "Poor posture, long sitting hours, muscle strain, and age-related spine changes are common causes of spine problems and chronic back pain. Homeopathic spine treatment helps address these issues naturally by targeting root causes.",
      },
      {
        question: "How long does recovery take?",
        answer:
          "Recovery varies by condition, but many patients experience gradual improvement with consistent homeopathic treatment for spine pain and proper lifestyle guidance.",
      },
      {
        question: "How does homeopathy help with spine problems?",
        answer:
          "Homeopathy for spine issues works by reducing inflammation, easing nerve compression, improving mobility, and strengthening the body's natural healing response for long-term spine pain relief.",
      },
      {
        question: "Does homeopathic spine treatment have side effects?",
        answer:
          "No. Homeopathic medicine for spine pain is gentle, safe, and suitable for long-term use without causing dependency or side effects—making it a preferred option for non-surgical spine care.",
      },
      {
        question: "Will I need to stay in the hospital?",
        answer:
          "No. Homeopathic spine treatment is completely non-invasive and does not require hospitalization.",
      },
    ],
    cta: {
      title: "Ready to Start Your Recovery Journey?",
      subtitle: "Schedule a consultation with our spine specialists today",
      buttonText: "Book Your Appointment Today",
    },
  },
  {
    key: "git",
    routes: ["git", "git-treatments"],
    tag: "Gastroenterology Homeopathic Consultant",
    title: "Comprehensive GIT Treatment Solutions",
    image: "/src/assets/treatments/GITTreatments.jpg",
    description:
      "Gentle, evidence-informed homeopathic care for digestive system disorders. Our team focuses on careful assessment, individualized remedies, and long-term support to help you improve your digestive health naturally.",
    services: {
      title: "Our GIT Treatment Services",
      subtitle:
        "Holistic gastrointestinal care using safe homeopathic remedies and mindful treatment methods.",
      services: [
        "IBS",
        "Acid Peptic Disease",
        "Gall Stones",
        "Liver Cirrhosis",
        "Fatty Liver",
        "Gastric Ulcers",
        "Gastritis",
        "Ulcerative Colitis",
        "Crohn’s Disease",
        "Pancreatitis",
        "Constipation",
        "Piles",
        "Fissure",
        "Fistula",
        "Anorexia",
        "Umbilical/Inguinal Hernia",
        "Rectal Prolapse",
      ],
    },
    symptoms: {
      title: "When to See a Gastroenterology Homeopathic Consultant",
      cardTitle:
        "Schedule a consultation if you experience any of these symptoms.",

      description:
        "Don’t ignore digestive discomfort. Early evaluation can help reduce complications and support better daily well-being. Schedule a consultation if you experience any of these symptoms.",

      symptoms: [
        "Chronic abdominal pain",
        "Persistent heartburn or acid reflux",
        "Difficulty swallowing",
        "Unexplained weight loss",
        "Changes in bowel habits",
        "Blood in stool",
        "Chronic diarrhea or constipation",
        "Nausea and vomiting",
        "Bloating and gas",
        "Loss of appetite",
      ],
    },
    treatmentApproach: [
      {
        step: 1,
        title: "Initial Assessment",
        description:
          "Detailed review of your medical history, lifestyle, and symptoms to understand the root imbalance affecting your digestive system.",
      },
      {
        step: 2,
        title: "Diagnostic Support",
        description:
          "We may recommend essential lab tests or reports (if needed) to gain clarity while planning your homeopathic digestive treatment.",
      },
      {
        step: 3,
        title: "Treatment Planning",
        description:
          "A customized homeopathic treatment plan designed around your symptoms, triggers, diet habits, and overall digestive health goals.",
      },
      {
        step: 4,
        title: "Ongoing Management",
        description:
          "Regular follow-ups, remedy adjustments, and guidance on routine and diet to support long-term digestive improvement.",
      },
    ],
    faqs: [
      {
        question: "How to improve gut health with homeopathy?",
        answer:
          "Homeopathy improves gut health by reducing inflammation, restoring digestive balance, and easing issues like acidity, bloating, IBS, and constipation. Personalized remedies strengthen overall gut function naturally.",
      },
      {
        question: "What is the best treatment for gut health?",
        answer:
          "A personalized homeopathic treatment plan is one of the best ways to improve gut health. It targets root causes such as acidity, poor digestion, or stress and helps regulate digestion for long-term relief.",
      },
      {
        question: "Does homeopathy work for gastric problems?",
        answer:
          "Yes. Homeopathy is effective for acidity, gas, bloating, indigestion, IBS, and chronic gastric problems. Remedies support natural digestion and reduce discomfort without side effects.",
      },
      {
        question: "How to improve digestion with homeopathy?",
        answer:
          "Homeopathy enhances digestion by calming gastric irritation, improving enzyme function, and regulating bowel movements. Combined with healthy eating habits, it supports smooth and lasting digestive health.",
      },
      {
        question: "How long does treatment take?",
        answer:
          "Every case is different. Some patients experience relief within weeks, while others may require longer-term care for chronic conditions. Homeopathy focuses on steady, root-based improvement rather than quick suppression of symptoms.",
      },
    ],
    cta: {
      title: "Take Control of Your Digestive Health",
      subtitle:
        "Consult our homeopathic GI specialists for gentle, personalized digestive care.",
      buttonText: "Book Your Appointment Today",
    },
  },

  {
    key: "cosmetology",
    routes: ["cosmetology"],
    tag: "Advanced Aesthetic Solutions",
    title: "Transform Your Appearance with Expert Cosmetology",
    image: "/src/assets/treatments/Cosmetology.jpg",
    description:
      "Experience the perfect blend of medical science and aesthetic artistry. Our advanced cosmetology treatments help you look and feel your absolute best, naturally.",
    services: {
      title: "Our Cosmetology Services",
      subtitle:
        "State-of-the-art aesthetic treatments customized to your unique beauty goals",
      services: [
        "Psoriasis",
        "Acne",
        "Hair Fall",
        "Dandruff",
        "Thinning of Hair",
        "Seborrheic Dermatitis",
        "Cellulitis",
        "Warts",
        "Corns",
        "Melasma",
        "Hyperpigmentation",
        "Leucoderma",
        "Allergies",
      ],
    },
    benefits: {
      title: "Treatment Benefits",
      cardTitle: "Treatment Benefits",
      description:
        "Our cosmetic treatments offer numerous benefits beyond aesthetic improvement.",
      expectations: [
        "Improved skin texture and tone",
        "Reduced fine lines and wrinkles",
        "Enhanced facial contours",
        "Minimized pores and blemishes",
        "Brighter, more radiant complexion",
        "Increased collagen production",
        "Long-lasting results",
        "Minimal downtime",
      ],
    },
    treatmentApproach: [
      {
        step: 1,
        title: "Personal Consultation",
        description:
          "Comprehensive skin analysis and discussion of your aesthetic goals and concerns.",
      },
      {
        step: 2,
        title: "Customized Plan",
        description:
          "Tailored treatment strategy designed specifically for your skin type and objectives.",
      },
      {
        step: 3,
        title: "Professional Treatment",
        description:
          "Expert application of advanced techniques in a comfortable, clinical setting.",
      },
      {
        step: 4,
        title: "Follow-up Care",
        description:
          "Ongoing support, maintenance recommendations, and progress monitoring.",
      },
    ],
    faqs: [
      {
        question: "Are cosmetic treatments safe?",
        answer:
          "Yes, all cosmetic treatments are performed using FDA-approved technologies under expert medical supervision.",
      },
      {
        question: "How long do cosmetic results last?",
        answer:
          "Results vary based on the procedure, skin type, and aftercare but typically last from several months to years.",
      },
    ],
    cta: {
      title: "Ready to Transform Your Appearance?",
      subtitle:
        "Book an appointment today and take the first step towards looking and feeling your absolute best",
      buttonText: "Book Your Appointment Today",
    },
  },
  {
    key: "kidney",
    routes: ["kidney-treatment"],
    tag: "Kidney Treatment",
    title: "Expert Kidney Treatment & Nephrology Care",
    image: "/src/assets/treatments/KidneyTreatment.jpg",
    description:
      "Comprehensive homeopathic nephrology care for better kidney health management, from prevention to advanced natural treatments. Our specialized team is dedicated to supporting, preserving, and improving your kidney function.",
    services: {
      title: "Our Kidney Treatment Services",
      subtitle:
        "Comprehensive nephrology care for all kidney-related conditions",
      services: [
        "Kidney Stone",
        "Nephritis",
        "Pyelonephritis",
        "Involuntary Urination",
        "Prostate Enlargement (BEP)",
        "Stricture",
        "Urinary Incontinence",
        "Nocturnal Urination",
        "Bed Wetting",
        "Recurrent UTI",
      ],
    },
    symptoms: {
      title: "When to See a Nephrologist",
      cardTitle: "Warning Signs ",
      description:
        "Early detection of kidney issues is vital. Consult our homeopathic nephrology specialists if you notice these warning signs or have risk factors like diabetes or high blood pressure.",
      symptoms: [
        "Decreased urine output",
        "Swelling in legs, ankles, or feet",
        "Persistent fatigue",
        "Shortness of breath",
        "Confusion or difficulty concentrating",
        "Chest pain or pressure",
        "Nausea and vomiting",
        "Blood in urine",
        "Foamy or bubbly urine",
        "High blood pressure",
      ],
    },
    treatmentApproach: [
      {
        step: 1,
        title: "Comprehensive Evaluation",
        description:
          "Detailed kidney function assessment, imaging review, and discussion of your symptoms using a holistic, homeopathic approach.",
      },
      {
        step: 2,
        title: "Customized Plan",
        description:
          "Tailored natural treatment strategy designed for your kidney condition, health goals, and long-term wellness.",
      },
      {
        step: 3,
        title: "Professional Treatment",
        description:
          "Expert application of safe homeopathic kidney care techniques in a supportive, clinical setting.",
      },
      {
        step: 4,
        title: "Follow-up Care",
        description:
          "Ongoing monitoring, lifestyle guidance, and progress evaluation to support better kidney health outcomes.",
      },
    ],
    faqs: [
      {
        question: "What are the early signs of kidney disease?",
        answer:
          "Early signs may include swelling, fatigue, decreased urine output, or changes in urination. A homeopathic kidney evaluation helps detect issues naturally and safely.",
      },
      {
        question: "How can I prevent kidney disease?",
        answer:
          "Regular hydration, a balanced diet, and routine kidney health checkups can help. Homeopathic prevention focuses on strengthening the body's natural healing response.",
      },
      {
        question: "Are homeopathic remedies safe for kidney patients?",
        answer:
          "Yes. Homeopathic remedies are gentle and safe for kidney patients, as they use natural, non-toxic formulations. They support overall kidney health, help ease symptoms, and work well alongside conventional kidney treatments without burdening the kidneys.",
      },
      {
        question: "Can kidney function be restored?",
        answer:
          "Kidney recovery depends on the severity of damage. Homeopathic treatment aims to support kidney function, reduce symptoms, and improve overall kidney health.",
      },
      {
        question: "Are homeopathic remedies safe for kidney patients?",
        answer:
          "Homeopathic kidney remedies are considered safe because they do not strain the kidneys and use mild, natural preparations. They help improve wellness, support kidney function, and reduce discomfort when paired with proper medical guidance.",
      },
    ],
    cta: {
      title: "Protect Your Kidney Health",
      subtitle: "Schedule a consultation with our nephrology specialists today",
      buttonText: "Book Your Appointment Today",
    },
  },
  {
    key: "gynecologist",
    routes: ["gynecologist-treatment"],
    tag: "Gynecology Treatment",
    title: "Comprehensive Gynecology & Women's Health Care",
    image: "/src/assets/treatments/Gynecologist Treatment.jpg",
    description:
      "Expert gynecological care at every stage of life. From adolescence through menopause, we offer compassionate, personalized support for women's health concerns using a holistic and patient-centered approach.",
    services: {
      title: "Our Gynecology Treatment Services",
      subtitle: "Complete women's healthcare from prevention to treatment",
      services: [
        "PCOD",
        "Leucorrhoea",
        "PCOS",
        "Fibroid",
        "Irregular Menses",
        "Amenorrhoea",
        "Menorrhagia",
        "Uterine Prolapse",
        "Pre and Post Pregnancy Care",
      ],
    },
    symptoms: {
      title: "When to See a Gynecologist",
      cardTitle: "Common Signs You Should Not Ignore",
      description:
        "You should consult a gynecologist if you notice unusual symptoms or changes in your reproductive health. Early evaluation helps you address concerns and improve long-term wellness.",
      symptoms: [
        "Irregular menstrual cycles",
        "Severe menstrual cramps or pelvic pain",
        "Excessive bleeding or clotting",
        "Pain during intercourse",
        "Unusual vaginal discharge or foul odor",
        "Itching, irritation, or discomfort",
        "Pregnancy-related concerns",
        "PCOS symptoms (weight gain, acne, excessive hair growth)",
        "Menopause symptoms (hot flashes, mood swings, night sweats)",
        "Difficulty conceiving",
      ],
    },
    treatmentApproach: [
      {
        step: 1,
        title: "Detailed Assessment",
        description:
          "A thorough consultation, including medical history, physical examination, and necessary diagnostic evaluation, to understand your concerns.",
      },
      {
        step: 2,
        title: "Personalized Treatment Plan",
        description:
          "A customized plan based on age, lifestyle, menstrual history, medical condition, and reproductive goals to support your long-term well-being.",
      },
      {
        step: 3,
        title: "Evidence-Based Treatment",
        description:
          "Holistic homeopathic care using safe remedies, lifestyle recommendations, and supportive medical guidance tailored to your needs.",
      },
      {
        step: 4,
        title: "Follow-Up & Care Monitoring",
        description:
          "Regular monitoring, preventive care, and ongoing support to promote consistent improvement and better women's health outcomes.",
      },
    ],
    faqs: [
      {
        question: "How often should I visit a gynecologist?",
        answer:
          "An annual check-up is usually recommended, but more frequent visits may be needed if you experience menstrual irregularities, pelvic pain, or pregnancy-related concerns.",
      },
      {
        question: "Can homeopathy help regulate irregular periods?",
        answer:
          "Homeopathy may help ease stress and discomfort linked to irregular periods. However, it should be paired with a medical evaluation to rule out conditions like thyroid issues, anemia, or PCOS.",
      },
      {
        question: "When should I be concerned about menstrual irregularities?",
        answer:
          "If cycles are consistently late, unusually heavy, too frequent, or accompanied by severe pain, consult a gynecologist for proper assessment.",
      },
      {
        question: "What are the signs of menopause?",
        answer:
          "Common signs include hot flashes, mood changes, irregular periods, night sweats, and sleep disturbances. A gynecologist can help manage symptoms and guide supportive care.",
      },
      {
        question: "Is homeopathy effective for PCOS and PCOD?",
        answer:
          "Homeopathy can support symptom relief, such as acne or cycle-related discomfort, but PCOS requires medical monitoring and lifestyle changes. Homeopathy works best as a complementary approach.",
      },
    ],
    cta: {
      title: "Your Health, Our Priority",
      subtitle: "Schedule a consultation with our women's health specialists",
      buttonText: "Book Your Appointment Today",
    },
  },
  {
    key: "migraine",
    routes: ["migraine-treatment"],
    tag: "Migraine Treatment",
    title: "Advanced Migraine Treatment & Management",
    image: "/src/assets/treatments/Migraine Treatment.jpg",
    description:
      "Expert care for chronic migraines and severe headaches. Our specialized approach combines the latest treatments with personalized care to help you live pain-free.",
    services: {
      title: "Our Migraine Treatment Services",
      subtitle:
        "Targeted care for migraine-related neurological conditions using a comprehensive treatment approach.",
      services: ["Migraine", "Headache", "Vertigo", "Brain Haemorrhage"],
    },
    symptoms: {
      title: "When to See a Specialist for Migraine",
      cardTitle: "Warning Signs You Should Not Ignore",
      description:
        "Persistent or recurring migraine symptoms should be evaluated early to prevent chronic headaches and improve long-term migraine management.",
      symptoms: [
        "Headaches lasting more than 4 hours",
        "Pulsating or one-sided headache pain",
        "Sensitivity to light, sound, or smell (common in chronic migraine)",
        "Nausea or vomiting during headache episodes",
        "Visual disturbances such as flashes, zig-zag lines, or blurred vision",
        "Tingling or numbness in the face or hands",
        "Difficulty speaking or focusing",
        "Headaches triggered by stress, lack of sleep, or hormonal changes",
        "Recurrent headaches more than 3–4 times per month",
      ],
    },
    treatmentApproach: [
      {
        step: 1,
        title: "Comprehensive Diagnosis",
        description:
          "Detailed assessment including medical history, lifestyle triggers, neurological evaluation, and imaging (if required) to identify migraine root causes for effective homeopathic treatment.",
      },
      {
        step: 2,
        title: "Personalized Treatment Plan",
        description:
          "Customized plan based on migraine type, severity, triggers, and patient lifestyle using targeted homeopathic remedies for long-term relief.",
      },
      {
        step: 3,
        title: "Targeted Treatment",
        description:
          "Medication support, stress management therapy, diet guidance, physical therapy, and advanced homeopathic interventions when required.",
      },
      {
        step: 4,
        title: "Follow-Up & Lifestyle Support",
        description:
          "Continuous monitoring, preventive strategies, and lifestyle guidance to reduce recurrence and improve long-term migraine control and quality of life.",
      },
    ],
    faqs: [
      {
        question:
          "What’s the difference between a migraine and a regular headache?",
        answer:
          "Migraines are more intense and often include nausea, light or sound sensitivity, and throbbing pain. Regular headaches are usually milder and shorter, while chronic migraines may require evaluation by a migraine specialist.",
      },
      {
        question: "What triggers migraines?",
        answer:
          "Common migraine triggers include stress, lack of sleep, hormonal changes, dehydration, certain foods, bright lights, and strong smells. Identifying personal triggers is an important part of migraine management and long-term headache control.",
      },
      {
        question: "Can migraine be cured with homeopathy?",
        answer:
          "Migraine cannot be permanently cured, but homeopathic treatment may help reduce symptom intensity, attack frequency, and recurrence over time when combined with lifestyle changes.",
      },
      {
        question: "Is migraine 100% curable?",
        answer:
          "Migraine cannot be permanently cured, but homeopathic migraine treatment may help reduce symptom intensity, attack frequency, and recurrence over time. It is used as a supportive, long-term approach along with lifestyle changes.",
      },
      {
        question: "What are the symptoms of a severe migraine?",
        answer:
          "Severe migraine symptoms include intense one-sided headache, nausea or vomiting, sensitivity to light and sound, visual disturbances, dizziness, and difficulty concentrating- often requiring medical attention for chronic migraine relief.",
      },
    ],
    cta: {
      title: "Find Relief from Chronic Migraines",
      subtitle:
        "Consult our migraine specialists for personalized and long-term headache management.",
      buttonText: "Book Your Appointment Today",
    },
  },
  {
    key: "ent",
    routes: ["ent-treatments"],
    tag: "ENT Treatments",
    title: "Expert ENT Treatment for Ear, Nose & Throat",
    image: "/src/assets/treatments/ENT Treatment.jpg",
    description:
      "Expert ENT care for all ear, nose, and throat conditions. From common infections to chronic sinus issues, allergies, or advanced throat concerns, our specialists provide precise evaluation and compassionate, personalized treatment to restore long-term ENT health.",
    services: {
      title: "Our ENT Treatment Services",
      subtitle:
        "Comprehensive care for ear, nose, throat, and related head and neck conditions.",
      services: [
        "Sinusitis",
        "Allergic Rhinitis",
        "DNS",
        "Recurrent Tonsillitis",
        "Nasal Polyp",
        "Laryngitis",
        "Adenoids",
        "Otitis Media",
        "Post-Nasal Drip",
        "Hoarseness of Voice",
      ],
    },
    symptoms: {
      title: "When to Visit an ENT Specialist",
      cardTitle: "Common Warning Signs",
      description:
        "Recognize early ENT symptoms to prevent chronic ear, nose, and throat issues and seek timely evaluation.",
      symptoms: [
        "Persistent ear pain or discharge",
        "Frequent sore throat or difficulty swallowing",
        "Hearing loss or ringing in the ears",
        "Blocked or stuffy nose for more than 2–3 weeks",
        "Snoring or breathing difficulty while sleeping",
        "Recurrent sinus infections",
        "Chronic cough or hoarseness of voice",
        "Dizziness, vertigo, or balance issues",
        "Allergies affecting breathing or sleep",
      ],
    },
    treatmentApproach: [
      {
        step: 1,
        title: "Detailed ENT Assessment",
        description:
          "Thorough ear, nose, and throat evaluation using diagnostic tools, medical history review, and symptom analysis to identify the root cause of ENT issues.",
      },
      {
        step: 2,
        title: "Customized Treatment Plan",
        description:
          "A tailored ENT treatment plan based on condition type such as sinusitis, allergies, tonsil problems, throat infections, nasal obstruction, or hearing-related concerns.",
      },
      {
        step: 3,
        title: "Advanced Treatment & Therapy",
        description:
          "Holistic ENT management with medication, allergy support, sinus care, throat treatment, hearing evaluation, and minimally invasive procedures when required.",
      },
      {
        step: 4,
        title: "Follow-Up & Prevention Support",
        description:
          "Regular monitoring, lifestyle guidance, and preventive measures to reduce flare-ups and support long-term ENT wellness.",
      },
    ],
    faqs: [
      {
        question: "Which medicine is best for ENT problems?",
        answer:
          " There is no single “best” medicine for ENT problems. Treatment depends on symptoms, age, severity, and triggers such as sinus infection, allergies, or throat issues. In homeopathy, remedies are selected individually to support symptom relief and overall ENT health, alongside proper medical evaluation.",
      },
      {
        question: "Do I need surgery for tonsillitis?",
        answer:
          "Surgery is considered only when tonsillitis becomes chronic or severely affects breathing or swallowing. Many cases of recurrent tonsillitis improve with medical management, monitoring, and supportive homeopathic ENT treatment without surgery.",
      },
      {
        question: "Is homeopathy safe for ENT problems in children?",
        answer:
          "Homeopathy is generally considered gentle and safe for children when prescribed by a qualified doctor. It may help support symptom comfort in mild ENT problems like ear pain or nasal congestion, but persistent symptoms should always be evaluated by an ENT or pediatric specialist.",
      },
      {
        question:
          "Can homeopathy help with a blocked nose and nasal congestion?",
        answer:
          "Homeopathy may help ease a blocked nose and nasal congestion by addressing triggers such as allergies, sinus congestion, or recurrent colds. It is often used as supportive care along with trigger avoidance and medical guidance for underlying ENT conditions.",
      },
      {
        question: "Can sinus problems come back after treatment?",
        answer:
          "With proper treatment, lifestyle care, and trigger management, many patients experience long-term relief from sinus problems. Regular follow-ups, allergy control, and supportive homeopathic care help reduce recurrence and maintain better sinus health over time.",
      },
    ],
    cta: {
      title: "Breathe Easy, Hear Clearly, Live Better",
      subtitle:
        "Schedule a consultation with our ENT specialists for personalized care.",
      buttonText: "Book Your Appointment Today",
    },
  },
  {
    key: "joint",
    routes: ["joint-treatments"],
    tag: "Joint Treatments",
    title: "Advanced Joint Treatment & Orthopedic Care",
    image: "/src/assets/treatments/JointTreatment.jpg",
    description:
      "Expert orthopedic care for joint conditions. From arthritis management to minimally invasive procedures, we help restore mobility and ease chronic joint pain with a personalized approach.",
    services: {
      title: "Our Joint Treatment Services",
      subtitle:
        "Comprehensive orthopedic solutions for joint pain relief, mobility improvement, and long-term musculoskeletal health.",
      services: [
        "Arthritis",
        "Osteoarthritis",
        "Rheumatoid Arthritis",
        "Calcaneal Spur",
        "Avascular Necrosis",
        "Carpal Tunnel Syndrome",
        "Tennis Elbow",
        "Frozen Shoulder",
        "Gout",
      ],
    },
    symptoms: {
      title: "When to Visit an Orthopedic Specialist",
      cardTitle: "Warning Signs You Should Not Ignore",
      description:
        "Orthopedic symptoms should not be ignored. Early evaluation helps prevent long-term joint damage, chronic inflammation, or loss of mobility.",
      symptoms: [
        "Persistent joint pain or swelling",
        "Morning stiffness lasting more than 30 minutes",
        "Difficulty walking, bending, or lifting",
        "Pain that worsens with movement",
        "Clicking, popping, or instability in joints",
        "Numbness or tingling in hands or feet",
        "Restricted shoulder, elbow, or knee movement",
        "Sudden severe joint pain",
        "Redness, warmth, or tenderness in joints",
        "Repeated injuries or chronic strain",
      ],
    },
    treatmentApproach: [
      {
        step: 1,
        title: "Detailed Orthopedic Evaluation",
        description:
          "Thorough assessment including joint mobility tests, X-ray findings, medical history, and functional analysis to clearly understand your condition.",
      },
      {
        step: 2,
        title: "Personalized Treatment Plan",
        description:
          "Customized plan based on your symptoms, daily activity levels, inflammation severity, and long-term mobility goals.",
      },
      {
        step: 3,
        title: "Advanced Treatment & Therapy",
        description:
          "Targeted care using safe homeopathic treatment support, joint-care therapies, lifestyle guidance, and mobility-focused rehabilitation strategies.",
      },
      {
        step: 4,
        title: "Follow-Up & Prevention Support",
        description:
          "Regular monitoring, progress tracking, and preventive care to support long-term joint health and reduce recurrence.",
      },
    ],
    faqs: [
      {
        question: "What causes joint pain?",
        answer:
          "Joint pain commonly occurs due to inflammation, arthritis, injury, or overuse. A proper evaluation helps identify the exact cause and guide the right treatment approach.",
      },
      {
        question: "When should I see a doctor for joint pain?",
        answer:
          "You should consult a specialist if joint pain lasts more than a week, worsens with movement, causes swelling, or begins affecting your daily routine or sleep.",
      },
      {
        question: "How long does recovery take with joint treatment?",
        answer:
          "Recovery depends on the condition and consistency of care. Many patients notice gradual improvement over weeks with appropriate treatment, lifestyle changes, and guided exercises.",
      },
      {
        question: "Can exercise help joint pain?",
        answer:
          "Yes. Gentle and guided exercises can reduce stiffness and strengthen joints. The ideal routine depends on the condition and should be recommended by a specialist.",
      },
      {
        question: "Are homeopathic treatments effective for joint problems?",
        answer:
          "Homeopathy may help ease joint discomfort, inflammation, and stiffness by supporting overall well-being. It works best when combined with proper medical guidance and movement therapy.",
      },
    ],
    cta: {
      title: "Ready to Regain Your Mobility?",
      subtitle:
        "Book a consultation today and take the first step toward pain-free movement and healthier joints.",
      buttonText: "Book Your Appointment Today",
    },
  },
  {
    key: "child",
    routes: ["child-treatments"],
    tag: "Child Treatments",
    title: "Compassionate Pediatric Care for Your Child",
    image: "/src/assets/treatments/ChildTreatment.jpg",
    description:
      "Expert medical care for infants, children, and adolescents. We focus on early detection, accurate diagnosis, and effective treatment to support healthy growth and development at every stage.",
    services: {
      title: "Our Child Treatment Services",
      subtitle:
        "Comprehensive healthcare for children from newborn to adolescence.",
      services: [
        "Recurrent Cold and Cough",
        "Recurrent Tonsillitis",
        "Asthma",
        "Pneumonia",
        "Whooping Cough",
        "Food Poisoning",
        "Diarrhoea",
        "Worm Infestation",
        "Fungal Infection",
        "Delayed Growth",
        "Delayed Speech",
        "Malnutrition",
      ],
    },
    when_to_consult: {
      title: "When to Consult a Pediatric Specialist",
      cardTitle: "Warning Signs You Should Not Ignore",
      description:
        "Visit a pediatrician if your child shows recurring symptoms, delayed milestones, or unusual behavior patterns.",
      symptoms: [
        "Repeated cough, cold, or fever",
        "Delayed speech or difficulty communicating",
        "Poor appetite or slow weight gain",
        "Frequent stomach aches or vomiting",
        "Behavioral changes, hyperactivity, or lack of focus",
        "Developmental delays such as walking, crawling, sitting, or talking",
        "Skin rashes or allergies",
        "Trouble sleeping or irritability",
        "School or learning difficulties",
      ],
    },
    treatmentApproach: [
      {
        step: 1,
        title: "Complete Child Assessment",
        description:
          "Growth evaluation, developmental screening, physical examination, and diagnosis.",
      },
      {
        step: 2,
        title: "Child-Specific Treatment Plan",
        description:
          "Age-appropriate treatment tailored to your child’s physical, emotional, or developmental needs.",
      },
      {
        step: 3,
        title: "Therapy & Advanced Care",
        description:
          "Physiotherapy, developmental therapy, speech therapy, behavioral therapy, allergy care, and immunity-boosting treatments.",
      },
      {
        step: 4,
        title: "Monitoring & Parent Guidance",
        description:
          "Regular check-ups, progress tracking, home-care instructions, and preventive care for long-term wellness.",
      },
    ],
    faqs: [
      {
        question: "Can we give homeopathy to kids?",
        answer:
          "Yes. Homeopathy is generally considered gentle and safe for children when prescribed by a qualified homeopathic doctor. It is often used to support common childhood conditions and overall well-being.",
      },
      {
        question: "Is homeopathic treatment safe for infants and toddlers?",
        answer:
          "When given under professional guidance, homeopathic medicines are safe for infants and toddlers. Remedies and dosage are selected carefully based on age, symptoms, and overall health.",
      },
      {
        question:
          "Can homeopathy help with recurrent cold, cough, or tonsillitis in children?",
        answer:
          "Homeopathy may help support children with recurrent cold, cough, or tonsillitis by improving immunity and reducing symptom discomfort. Persistent or severe symptoms should always be medically evaluated.",
      },
      {
        question: "Can homeopathy help improve a child’s immunity?",
        answer:
          "Homeopathy may support a child’s overall immunity and well-being when used alongside proper nutrition, sleep, hygiene, and regular pediatric care.",
      },
      {
        question: "When should I bring my child to a doctor?",
        answer:
          "You should consult a doctor if your child has persistent fever, breathing difficulty, repeated infections, poor appetite, delayed milestones, unusual behavior, or symptoms that do not improve within a few days.",
      },
    ],
    cta: {
      title: "Care Your Child Deserves, Every Step of the Way",
      subtitle:
        "Schedule a pediatric consultation to support your child’s healthy growth and development.",
      buttonText: "Book Your Appointment Today",
    },
  },
];

// Helper function to get treatment data by route
export const getTreatmentByRoute = (routePath) => {
  // Extract the route segment (e.g., "spine-treatments" from "/treatments/spine-treatments")
  const routeSegment = routePath.split("/").pop() || "";

  return TREATMENTS_DATA.find((treatment) =>
    treatment.routes.includes(routeSegment)
  );
};
